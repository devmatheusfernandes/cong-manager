...
export const PUBLICADORES_LIST: Publicador[] = [
  ...
  { "id": "9d02275b-04f4-4e5a-96f5-edbc21fc62f3", "nome": "Antonio" },
...
export function findPublicadorByName(nome: string): Publicador | null {
  ...
  const exactMatch = PUBLICADORES_LIST.find(p => 
    p.nome.toLowerCase() === nomeNormalizado
  );
  
  if (exactMatch) return exactMatch;
  
  // Busca aproximada (contém o nome)
  const partialMatch = PUBLICADORES_LIST.find(p => 
...
export const PUBLICADORES_PROMPT_LIST = PUBLICADORES_LIST
  .map(p => `"${p.nome}": "${p.id}"`)
  .join(',\n  ');