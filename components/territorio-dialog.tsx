...
export function TerritorioDialog({ territorio, onSuccess, trigger }: TerritorioDialogProps) {
  const [open, setOpen] = useState(false)
  const [loading, setLoading] = useState(false)
  const [formData, setFormData] = useState({
    nome: territorio?.nome || "",
    coordenadas: territorio?.coordenadas ? JSON.stringify(territorio.coordenadas, null, 2) : "",
  ...
  useEffect(() => {
    if (!isEditing && !formData.congregacao_id) {
      ...
      setFormData(prev => ({ ...prev, congregacao_id: "660e8400-e29b-41d4-a716-446655440001" }))
    }
    ...
    try {
      ...
      if (formData.coordenadas.trim()) {
        try {
          ...
          toast.error("Formato de coordenadas inválido. Use um GeoJSON válido.")
          setLoading(false)
          return
        }
      }
      ...
      const url = isEditing ? `/api/territorios/${territorio.id}` : '/api/territorios'
      ...
      toast.success(isEditing ? 'Território atualizado com sucesso!' : 'Território criado com sucesso!')
      setOpen(false)
      onSuccess()
      ...
      toast.error(error instanceof Error ? error.message : 'Erro ao salvar território')
    } finally {
      setLoading(false)
    }
  }
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              onChange={(e) => setFormData(prev => ({ ...prev, coordenadas: e.target.value }))}
              placeholder={`{
  "type": "Polygon",
  "coordinates": [[
    [-46.6333, -23.5505],
    ...
    [-46.6333, -23.5505]
            ...
            <p className="text-xs text-muted-foreground">
              Opcional. Insira um GeoJSON válido para definir a área do território.
            </p>
          </div>
