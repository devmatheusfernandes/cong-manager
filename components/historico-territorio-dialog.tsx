...
export function HistoricoTerritorioDialog({
  ...
  const fetchHistorico = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`/api/territorios/${territorioId}/historico`);
      if (response.ok) {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'ativo':
        return <Badge variant="default" className="bg-amber-100 text-amber-800">Ativo</Badge>;
      case 'finalizado':
        return <Badge variant="secondary" className="bg-teal-100 text-teal-800">Finalizado</Badge>;
      case 'cancelado':
        return <Badge variant="destructive" className="bg-rose-100 text-rose-800">Cancelado</Badge>;
      default:
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium">{designacao.publicador.nome}</span>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <span className="font-medium">
                        {format(new Date(designacao.data_inicio), "dd/MM/yyyy", { locale: ptBR })}
                      </span>
                    </div>
                  </div>
                  ...
                  {designacao.data_fim && (
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <span className="font-medium">
                          {format(new Date(designacao.data_fim), "dd/MM/yyyy", { locale: ptBR })}
                        </span>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <span className="font-medium">
                        {calcularDuracao(designacao.data_inicio, designacao.data_fim)}
                      </span>
                    </div>
                  </div>
                </div>
